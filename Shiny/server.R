...
shinyServer(function(input, output) {
	...
	output$summary <- renderUI({
		if (is.numeric(input$temperature)){
				...
				} else if (66 <= input$temperature & input$temperature < 79){
					HTML(paste("<h2>Today's clothing should include:</h2>",
						"<h4>**Long Sleeves</h4>", 
						"<h4>**Sweater</h4>", sep="<br/>"))
				} else if (80 <= input$temperature & input$temperature <= 110){
					HTML(paste("<h2>Today's clothing should include:</h2>",
					 "<h4>**Short Sleeves</h4>",
					 "<h4>YAY! It is FINALLY summer</h4>", sep="<br/>"))
				}
		} else if (is.na(input$temperature)){
			HTML("")
		}
		else if (!is.numeric(input$temperature) & !is.na(input$temperature)){
			HTML("Please put in a correct number for the temperature")
		}
	})
})